...
void Cuboid::saveToFile(const string &filename) {
    ...
    // Define edges of the cuboid (no diagonals)
    int edges[12][2] = {
    ...
    // Add a blank line to separate the cuboid's data from other shapes
    file << "\n";
...
void Cuboid::plotCuboid(const string &filename)
{
    ...
    // Ensure GNUPLOT renders only the edges without diagonals
    gnuplot << "splot '" << filename << "' with lines linecolor 'blue' linewidth 2\n";
    gnuplot << "pause -1\n";
    gnuplot.close();
